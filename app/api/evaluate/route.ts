...
// ─── Tool 스키마 (구조적 JSON 출력 강제) ──────────────────────────────────────
const EVAL_TOOL = {
  name: 'evaluate_paper',
  description: '기고문을 심사하고 평가 결과를 반환합니다.',
  input_schema: {
    type: 'object',
    properties: {
      total_score: { type: 'number', description: '총점 (0-100)' },
      recommendation: { type: 'string', enum: ['accept', 'revision', 'reject'] },
      scores: {
        type: 'object',
        properties: {
          topic_relevance:       { type: 'object', properties: { score: { type: 'number' }, max: { type: 'number' }, comment: { type: 'string' } }, required: ['score', 'max', 'comment'] },
          logic_structure:       { type: 'object', properties: { score: { type: 'number' }, max: { type: 'number' }, comment: { type: 'string' } }, required: ['score', 'max', 'comment'] },
          academic_contribution: { type: 'object', properties: { score: { type: 'number' }, max: { type: 'number' }, comment: { type: 'string' } }, required: ['score', 'max', 'comment'] },
          format_compliance:     { type: 'object', properties: { score: { type: 'number' }, max: { type: 'number' }, comment: { type: 'string' } }, required: ['score', 'max', 'comment'] },
          references:            { type: 'object', properties: { score: { type: 'number' }, max: { type: 'number' }, comment: { type: 'string' } }, required: ['score', 'max', 'comment'] },
        },
        required: ['topic_relevance', 'logic_structure', 'academic_contribution', 'format_compliance', 'references'],
      },
      executive_summary: {
        type: 'object',
        properties: {
        ...
        required: ['background', 'main_argument', 'evidence', 'conclusion', 'policy_implication'],
      },
      ...
      plagiarism: {
        type: 'object',
        properties: {
          similarity_estimate: { type: 'number' },
          risk_level:          { type: 'string', enum: ['low', 'medium', 'high'] },
          suspicious_passages: { type: 'array', items: { type: 'object', properties: { text: { type: 'string' }, reason: { type: 'string' }, risk: { type: 'string' } }, required: ['text', 'reason', 'risk'] } },
          ethics_note:         { type: 'string' },
        },
        required: ['similarity_estimate', 'risk_level', 'suspicious_passages', 'ethics_note'],
      },
      feedback_roadmap: {
        type: 'array',
        items: {
          ...
          required: ['priority', 'category', 'issue', 'suggestion', 'effort'],
        },
      },
    },
    required: ['total_score', 'recommendation', 'scores', 'executive_summary', 'strengths', 'weaknesses', 'plagiarism', 'feedback_roadmap'],
  },
} as const
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Claude API 호출 (tool_use로 구조적 JSON 강제) ────────────────────────
    ...
    const message = await client.messages.create({
      ...
      tools: [EVAL_TOOL],
      tool_choice: { type: 'tool', name: 'evaluate_paper' },
    ...
    if (!toolUse) {
      return NextResponse.json({ error: 'AI가 평가 결과를 반환하지 않았습니다.' }, { status: 422 })
    }

    let parsed: Omit<EvaluationResult, 'generated_at' | 'model_used'>
    try {
      parsed = toolUse.input as Omit<EvaluationResult, 'generated_at' | 'model_used'>
    } catch {
      return NextResponse.json({ error: 'AI 응답 파싱에 실패했습니다.' }, { status: 422 })
    }
