...
투고된 논문을 아래 기준에 따라 엄격하고 공정하게 심사하세요.
...
위 논문에 대해 아래 JSON 스키마에 정확히 맞춰 응답하세요:
{
...
function buildDemoResult(title: string, text: string): EvaluationResult {
  ...
  return {
    ...
    scores: {
      topic_relevance: {
        ...
        comment: hasMilitary
          ...
          : '논문 주제가 군사학 관련 분야와의 연관성이 다소 부족합니다. 군사·국방 관점을 더 명확히 제시할 필요가 있습니다.',
      },
      logic_structure: {
        score: logicScore, max: 25,
        comment: len > 30000
          ? '논리적 흐름이 전반적으로 양호하며 각 장의 연결이 자연스럽습니다. 주장을 뒷받침하는 근거 제시가 체계적입니다.'
          : '논문 구성은 적절하나 일부 주장에 대한 논거가 불충분합니다. 각 절 간의 논리적 연결을 보강하면 완성도가 높아질 것입니다.',
      },
    ...
    feedback_roadmap: [
      { priority: 'high', category: '형식 수정', issue: hasAbstract ? '참고문헌 형식 불일치' : '초록 누락', suggestion: hasAbstract ? '인용 형식을 APA 또는 학회 규정에 맞게 통일하세요' : '한글 초록(400자)과 영문 Abstract(200단어)를 작성하여 논문 앞부분에 추가하세요', effort: '2~3시간' },
  ...
  try {
    ...
    if (!text || text.trim().length < 100) {
      return NextResponse.json(
        { error: '평가할 논문 텍스트가 너무 짧습니다. (최소 100자)' },
        { status: 400 }
      )
    }
